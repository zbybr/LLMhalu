...
OUTPUT_DATA = f"/home/mdafifal.mamun/research/LLMhalu/mistral/data/mistral_outputs_truthfulqa_token_usage_seed{SEED}.csv"
...
mistral_model = Mistral(MODEL_ID, temperature=temperature)
...
def run_pipeline(df, output_path):
    ...
    for index, row in tqdm(df.iterrows(), total=len(df), desc="Processing issue"):
        ...
        base_response, base_token_usage = mistral_model.invoke(
        ...
        generated_samples = []
        for i in range(10):
            base_response, token_usage = mistral_model.invoke(
        ...
        response, token_usage = mistral_model.invoke(
